...
// Function to render the navigate button
...
function getTotalRecords($db) {
    $sql_count = "SELECT COUNT(*) as total FROM vehicle_development";
...
function getPaginatedData($db, $records_per_page, $offset) {
    $sql = "SELECT * FROM vehicle_development LIMIT $records_per_page OFFSET $offset";
    return $db->query($sql);
}

// Function to render the data table
function renderTable($resql, $db) {
    // Define global notranslate words
    $notranslate_word_ID = '<span class="notranslate"> ID</span>';
    // Display Columns
    print '<h1>Vehicles Table</h1>';
    print '<table class="noborder" width="100%">';
    print '<tr class="liste_titre">';
//     print '<th style="font-weight: bold;">' . $notranslate_word_ID . '</th>';
    print '<th style="font-weight: bold;"> Vehicle' . $notranslate_word_ID . '</th>';
    print '<th style="font-weight: bold;">Capacity</th>';
    print '<th style="font-weight: bold;">Cost per Trip</th>';
    ...
    while ($obj = $db->fetch_object($resql)) {
        print '<tr>';
//         print '<td>'.$obj->id.'</td>';
        print '<td>'.$obj->vehicle_id.'</td>';
        print '<td>'.$obj->capacity.'</td>';
        print '<td>'.$obj->cost_per_trip.'</td>';
...
// Render page header
renderPageHeader("Vehicles List");
...
if ($resql) {
    ...
    print "Error fetching data from vehicle_development table.";
}
