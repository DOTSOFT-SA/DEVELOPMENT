...
// Function to render the data table for routes
function renderRouteTable($resql, $db) {
    // Define notranslate words
    $notranslate_word_ID = '<span class="notranslate"> ID</span>';
    // Display Columns
    print '<h1>Routes Table</h1>';
    print '<table class="noborder" width="100%">';
    print '<tr class="liste_titre">';
//     print '<th style="font-weight: bold;">' . $notranslate_word_ID . '</th>';
    print '<th style="font-weight: bold;">Route ' . $notranslate_word_ID . '</th>';
    print '<th style="font-weight: bold;">Distance</th>';
    print '<th style="font-weight: bold;">Traffic Factor</th>';
    print '<th style="font-weight: bold;">Source Location ' . $notranslate_word_ID . '</th>';
    print '<th style="font-weight: bold;">Destination Location ' . $notranslate_word_ID . '</th>';
    ...
    while ($obj = $db->fetch_object($resql)) {
        print '<tr>';
//         print '<td>'.$obj->id.'</td>';
        print '<td>'.$obj->route_id.'</td>';
        print '<td>'.$obj->distance.'</td>';
        print '<td>'.$obj->traffic_factor.'</td>';
        print '<td>'.$obj->source_location_id.'</td>';
        print '<td>'.$obj->destination_location_id.'</td>';
...
$sql_count = "SELECT COUNT(*) as total FROM route_development";
$resql_count = $db->query($sql_count);
$total_records = $db->fetch_object($resql_count)->total;
$total_pages = ceil($total_records / $records_per_page);

$sql = "SELECT * FROM route_development LIMIT $records_per_page OFFSET $offset";
$resql = $db->query($sql);

if ($resql) {
    renderRouteTable($resql, $db);
    renderPagination($page, $total_pages);
} else {
    print "Error fetching data from route_development table.";
}
