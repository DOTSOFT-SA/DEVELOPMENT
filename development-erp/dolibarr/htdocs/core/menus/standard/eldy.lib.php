...
function get_left_menu_products($mainmenu, &$newmenu, $usemenuhider = 1, $leftmenu = 'none', $type_user = 0)
{
    ...
    if ($mainmenu == 'products') {
        ...
        if (isModEnabled('product')) {
            // Variable for "Development" word (to not be translated)
            $notranslate_word_development = '<span class="notranslate">Development</span>';
            # Add a new main section called "Development"
            $newmenu->add("/product/index.php?leftmenu=development", $langs->trans($notranslate_word_development), 0, $user->hasRight('product', 'read'), '', $mainmenu, 'development', 0, '', '', '', img_picto('', 'product', 'class="paddingright pictofixedwidth"'));
            // Add "List SKU" as a subsection under "Development"
            $newmenu->add("/product/list_sku.php?leftmenu=development", $langs->trans("SKU Orders"), 1, $user->hasRight('product', 'read'));
            // Add "Inventory Params" as a subsection under "Development"
            $newmenu->add("/product/list_inventory.php?leftmenu=development", $langs->trans("Inventory Params"), 1, $user->hasRight('product', 'read'));
            // Add a new section "Distribution / Routing" under "Development"
            $newmenu->add("/product/index.php?leftmenu=distribution", $langs->trans("Distribution / Routing"), 1, $user->hasRight('product', 'read'), '', $mainmenu, 'distribution', 0, '', '', '', img_picto('', 'distribution', 'class="paddingright pictofixedwidth"'));
            // Add sub-sub-sections under "Distribution / Routing"
            $newmenu->add("/product/list_vehicles.php?leftmenu=distribution", $langs->trans("Vehicles"), 2, $user->hasRight('product', 'read'));
            $newmenu->add("/product/list_routes.php?leftmenu=distribution", $langs->trans("Routes"), 2, $user->hasRight('product', 'read'));
            $newmenu->add("/product/list_locations.php?leftmenu=distribution", $langs->trans("Locations"), 2, $user->hasRight('product', 'read'));
        }
		// Products
		if (isModEnabled('product')) {
